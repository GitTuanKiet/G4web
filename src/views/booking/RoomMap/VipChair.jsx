...
const NormalChair = ({ chair, className }) => {
  ...
  const color = disabled ? 'bg-gray-500 opacity-60' : 'bg-yellow-500 opacity-100'
  const priceChair = showtime.price + showtime.price / 2
  ...
  return (
    <div className={className}>
      <button
        className={`w-10 h-10 shadow-md rounded-xl text-[10px] ${color} ${isChecked && 'opacity-60'} transition-opacity duration-300`}
        disabled={disabled}
