...
const RoomMap = () => {
  ...
  const roomMap = cinemas.find((item) => item.id === showtime.cinemaId)?.roomMap
        ...
        <div className="flex flex-col gap-6 justify-center items-center">
          ...
          <div className="grid grid-cols-12 gap-4">
            {normalChairs?.map((item, index) => {
              return (
                <NormalChair key={index} chair={item.chair}
          ...
          <div className="grid grid-cols-12 gap-4">
            {vipChairs?.map((item, index) => {
              return (
                <VipChair key={index} chair={item.chair}
                  // className={index % 2 === 0 ? 'ml-8' : ''}
                />
              )
            })}
          ...
          <div className="grid grid-cols-7 gap-4 mx-auto">
            {coupleChairs?.map((item, index) => {
              return (
                <CoupleChair key={index} chair={item.chair}
...
const SeatLegend = () => {
  return (
    <div className="px-4 py-2 w-full bg-gray-300 text-xs gap-4 flex justify-evenly">
      {seatTypes.map((seatType, index) => {
        return (
          <div key={index} className="flex gap-2">
            <div className={`h-10 rounded-xl ${seatType.style} flex items-center justify-center`}>
              {seatType.value}
            </div>
